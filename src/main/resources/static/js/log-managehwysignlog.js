$("#managesigninputBtn").click(function(e){
    e.preventDefault();
    window.location.href = "/signaturelog/active/";
});


function loadsignlogdetail(logID){
	window.location.href = "/signaturelog/active/"+logID;
}
...
$("#printPreviewBtn").click(function(e){
    ...
    $("#reportinfoModalTitle").append("<h4 class='modal-title'><i class='fa fa-print'></i> </span> Signature Log Report </h4> ")
    $("#reportinfoModal").modal('show');
	$("#reportinfopdf").css("height",$(window).height()-$(window).height()/4);
	$('#reportinfopdf').html("<iframe name='your_frame_name' src='/signreport/"+seqArr+"'  height=100% width=100%> </iframe>")
    $("#maskloader").hide();
    }
});
...
////ActionTaken////
//////////////////////////////////////////
function signactiontakenList_by(logID){
defaultArray=new Array();
    $.ajax({
        url: "/signaturelog/actiontakenList/"+logID,
        dataType: "json",
...
function signactiontakenList_update(logID,logActionId,ActionType,formDate,actionTaken,Delete){
    if (ActionType!="" && formDate!=""  && actionTaken!=""){
        ...
        $.ajax({
            url: "/signaturelog/actiontakenwrite/"+logID,
            dataType: "json",
            ...
            $('#signlogactionTakenTable').jexcel('setData', signactiontakenList_by(logID));
            }
        });
	}
}
...
/* Action Taken */
function inispreadsheetsignactionTaken() {
var ActionType = initcodearray_by(9,"");
data03=signactiontakenList_by($("#logID").val());
      ...
      var changed03 = function(instance, cell, x,y) {
          ...
          signactiontakenList_update($("#logID").val(),logActionId,ActionType,formDate,actionTaken,Delete);
       };

       if ($("#logID").val()=="0"){
       $('#signlogactionTakenTable').html("<p style='text-align:center;padding:4rem;'>Please enter the basic information first.</p>");
       $('#loggactionTakenmemo').empty();
       } else {
       $('#loggactionTakenmemo').empty();
       $('#loggactionTakenmemo').html("* In/Out ,Date and Action must be entered for registration. It is automatically saved.");
        $('#signlogactionTakenTable').jexcel({
            data:data03,
            width:"100%",
            minSpareRows:1,
...
$("#deldataBtn").click(function(e){
	...
	$("#maskloader").show();
    $.ajax({
        url: "/signaturelog/delete/"+logID,
        xhrFields: {withCredentials: true},
        complete: function (res) {
            window.location.href = "/signaturelog/list";
			$("#maskloader").hide();
        }
	});
	}
